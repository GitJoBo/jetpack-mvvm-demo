...
class CollectAdapter(data: MutableList<CollectBean>) : BaseDelegateMultiAdapter<CollectBean, BaseViewHolder>(data) {

    //文章类型
    private val Ariticle = 1
    //项目类型 本来打算不区分文章和项目布局用统一布局的，但是布局完以后发现差异化蛮大的，所以还是分开吧
    private val Project = 2
    private var collectAction: (item: CollectBean, v: CollectView, position: Int) -> Unit =
    ...
    init {
        ...
        getMultiTypeDelegate()?.let {
            it.addItemType(Ariticle, R.layout.item_article)
